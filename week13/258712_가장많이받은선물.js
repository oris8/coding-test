...
const solution = (friends, gifts) => {
  ...
  const giftIndexMap = {}; // { name: { friendName : 선물지수}, ... } 형태로 친구별 선물지수를 기록할 객체입니다
  const totalGiftIndexMap = {}; // { name: 선물지수 }, ... } 형태로 total선물지수를 기록할 객체입니다
  const result = new Array(friends.length).fill(0); // 계산 결과를 담을 배열

  // 친구목록을 돌면서 giftIndexMap 초기화
  friends.forEach((me) => {
    giftIndexMap[me] = {};
    friends.forEach((friend) => {
      giftIndexMap[me][friend] = 0;
    });
  });

  // gifts를 돌면서 각각의 친구별 선물지수 계산
  gifts.forEach((gifts) => {
    const [giver, recipient] = gifts.split(" ");
    giftIndexMap[giver][recipient] += 1;
    giftIndexMap[recipient][giver] += -1;
  });

  // 친구목록을 돌면서 giftIndexMap 값을 이용해서 total선물지수 계산
  friends.forEach((friend) => {
    totalGiftIndexMap[friend] = Object.values(giftIndexMap[friend]).reduce(
      (acc, index) => (acc += index),
      0
    );
  });

  // 문제에서 요구했던 계산 로직 (선물지수따라서 받는 선물 개수 result에 저장)
  friends.forEach((me, idx) => {
    friends.forEach((friend) => {
      if (me === friend) return;

      const giftIndex = giftIndexMap[me][friend];
      if (giftIndex > 0) return (result[idx] += 1);
      if (giftIndex === 0 && totalGiftIndexMap[me] > totalGiftIndexMap[friend])
        return (result[idx] += 1);
    });
  });
